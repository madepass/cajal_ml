%% Data path
save_name = 'dataSorted_0_left.mat';
...
hands = {'Left', 'Right'}; %'Left'
precision_angles = {'Precision_0', 'Precision_45', 'Precision_90', 'Precision_135'}; % '0','45','90','135'};
...
remove_short_trials = true;  % remove trials with motor states shorter than sample_duration
remove_weird_trials = true; % remove trials with weird/missing event names
%% Preprocessing Pipeline
for h_ = 1:length(hands)
    for pa_ = 1:length(precision_angles)
        hand = hands{h_};
        precision_angle = precision_angles{pa_};
        angle = split(precision_angle, '_');
        angle = angle{2};
        save_name = ['data_', lower(hand), '_', angle, '.mat'];
        fprintf(['Processing ', save_name, '...\n'])
        ...
        event_names = {[hand, ' hand Precision_CueOn'], [hand, ' hand Precision_CueOff'], [hand, ' hand Precision_GraspStart']};
        event_times = zeros([length(files_to_load_right), length(event_names)]);
        weird_event_trials = zeros([1, length(graspStarts)]);
        ...
        for f = 1:length(files_to_load_right)
            if f == 1
                ...
                events_ = {Events(:).label};
                event_inds = zeros([1, length(event_names)]);
                try % skip trials that have missing/nonstandard event names
                    for e = 1:length(event_names)
                        event_inds(e) = find(strcmp(events_, event_names(e)));
                    end
                catch 
                    weird_event_trials(f) = 1;
                    continue
                end
                event_times(f,:) = [Events(event_inds).times];
        ...
        baseline_short = find(cueOns+Time(end) < sample_duration);
        pre_grasp_short = find((cueOffs - cueOns) < sample_duration);
        reach_short = find((graspStarts + - cueOffs) < sample_duration);
        post_grasp_short = find((Time(end) - (graspStarts + sample_duration))<sample_duration);
        short_trials = [baseline_short, pre_grasp_short, reach_short, post_grasp_short];
        
        if remove_short_trials || remove_weird_trials 
            weird_trials = find(weird_event_trials);
            remove_inds = [];
            if remove_short_trials
                remove_inds = short_trials;
            end
            if remove_weird_trials
                remove_inds = [remove_inds, weird_trials];
            end
            lfpData_right(:, :, remove_inds) = [];
            cueOns(remove_inds) = [];
            cueOffs(remove_inds) = [];
            graspStarts(remove_inds) = [];
            n_trials = length(graspStarts);
            fprintf('Removed short and/or weird trials...\n')
        end
        ...
        data.spikes = spikes;
        data.hand = hand;
        data.alignedTo = aligned_to;
        data.angle = precision_angle;
